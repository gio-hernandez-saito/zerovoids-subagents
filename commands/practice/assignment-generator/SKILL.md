...
description: FE 과제 전형 문제를 생성합니다 (중복 방지, 난이도 표기, 사용자가 지정한 작업 디렉토리에 출력). "과제 하나 줘", "assignment", "과제 만들어줘" 등의 요청 시 이 스킬을 사용합니다.
version: 0.0.1
layer: command
...
2. **중복 방지**: 사용자가 지정한 작업 디렉토리(`$ARGUMENTS`, 미지정 시 사용자에게 질문) 내 기존 과제 frontmatter를 스캔
3. 요구사항 명세서 생성
...
과제는 사용자가 지정한 작업 디렉토리에 서브디렉토리로 생성합니다. 스킬은 디렉토리 경로를 하드코딩하지 않으며, 경로는 `$ARGUMENTS` 또는 대화 컨텍스트에서 받습니다.
