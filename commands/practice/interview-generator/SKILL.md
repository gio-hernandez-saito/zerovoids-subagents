...
2. 레퍼런스에서 질문 선별
3. **중복 방지**: 사용자가 지정한 작업 디렉토리(`$ARGUMENTS`, 미지정 시 사용자에게 질문) 내 기존 파일의 frontmatter를 스캔
4. 질문 1~5개 출제 (기본 3개)
...
질문은 사용자가 지정한 작업 디렉토리에 마크다운 파일로 저장합니다. 스킬은 디렉토리 경로를 하드코딩하지 않으며, 경로는 `$ARGUMENTS` 또는 대화 컨텍스트에서 받습니다.
